...
#
# if mode=="kmeans":
#    cos_list = [torch.nn.functional.cosine_similarity(sent_, r_list, dim=1).half() for sent_ in tqdm(normalized)]
# else:
#    cos_list = [torch.Tensor([cos(sent_, r_vec) for r_vec in r_list]) for sent_ in tqdm(normalized)]
#

# if mode=="kmeans":
#     cos_list = [torch.nn.functional.cosine_similarity(sent_, r_list, dim=1) for sent_ in tqdm(normalized)]
# elif mode in  ['rand','sent']:
#     cos_list = [torch.nn.functional.cosine_similarity(sent_, r_list, dim=1) for sent_ in tqdm(normalized)]
#     # cos_list = [torch.Tensor([cos(sent_, r_vec) for r_vec in r_list]) for sent_ in tqdm(normalized)]
...
# if mode not in ['base']:
#     cos_list = [torch.nn.functional.cosine_similarity(sent_, r_list, dim=1) for sent_ in tqdm(normalized)]
#     sq_cos =[(1-c.square()).sqrt() for c in cos_list]
...
for text in query_list:
   ...
   if mode == "base":
       ...
       #             similarity_.append(
       #                 cos(pt_load[i], query_embedding[0].flatten()))  # calculate similarity for each sentence in file
       #
       ...
       #
       # sq_cos_q = (1-cos_q_list.square()).sqrt()
       #
       # # initalize similaritylist
       #
       #
       # for k in tqdm(range(0, len(cos_list))):
       #
       #     # max_sim = -(cos_list[k] - cos_q_list).abs().max() + 1.000
       #
       #     max_sim = (cos_list[k]*cos_q_list + (sq_cos[k] *sq_cos_q)).min()
